...
# ==========================================
# 第三部分：启动训练并开启 TensorBoard 监控
# ==========================================
if __name__ == "__main__":
    print("初始化强化学习环境...")
    env = ROVP2PWrapper(rov_config)
    
    print("正在构建 PPO 神经网络并开启 TensorBoard 记录...")
    # 👉 核心：这里加入了 tensorboard_log 参数
    # model = PPO("MlpPolicy", env, verbose=1, learning_rate=0.0003, tensorboard_log="./rov_tensorboard/")
    model = PPO("MlpPolicy", env, verbose=1, learning_rate=0.0003, gamma=0.995, ent_coef=0.01, tensorboard_log="./rov_tensorboard/")
    
    checkpoint_callback = CheckpointCallback(
        save_freq=10000, 
        save_path='./rov_models/',
        name_prefix='ppo_rov_p2p'
    )
    
    print("🔥 开始训练！在训练时，你可以打开一个新的控制台来实时查看曲线。")
    model.learn(total_timesteps=1000000, callback=checkpoint_callback) # 这种复杂奖励建议训练长一点
    
    print("✅ 训练完成，正在保存最终模型...")
    model.save("ppo_rov_p2p_final")